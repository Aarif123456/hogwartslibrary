...
function addReservation() {
    ...
    var url =
        'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/reservation/addReservation';
...
function deleteReservation() {
    ...
    //create parameter to send to server side
    var par =
        'courseID=' + courseID + '&bookISBN=' + bookISBN + '&userID=' + profID;

    //Ajax insert
    var xmlhttp = new XMLHttpRequest();
    var url =
        'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/reservation/deleteReservation';
    xmlhttp.open('POST', url, true);
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            document.getElementById('hint').innerHTML = this.responseText;
...
function displayForm() {
  ...
  id="reservationForm" onsubmit="return false;">`;
    ...
    if (mode[1].checked) {
        //if delete form
        reservationForm.innerHTML += `<div id="bookISBNMenu">
      </div>
      <br>`;
        if (profID != 0 && courseID != -1) {
...
function loadReserveList(getList, listName) {
    ...
    xmlhttp.send(
        'listType=' + listName + '&courseID=' + courseID + '&userID=' + profID
    );
}
...
function renderProfessorCourses(update) {
    ...
    if (update || courseList[profID] == undefined) {
        ...
        try {
            document.getElementById('courseMenu').innerHTML =
                courseList[profID];
            displayForm(); //display form after courses have loaded
        } catch (e) {
            //if course menu is gone
            console.log(e);
...
function getProfessors(professorJSON) {
    if (professorJSON == 'No rows') {
        document.getElementById(
            'professorMenu'
        ).innerHTML = `This library system has no professor:(
...
function setNumberCopies() {
    var numCopies = parseInt(
        document.getElementById('numCopies').value.trim(),
        10
    );
    if (isNaN(numCopies)) {
        numCopies = 0;
