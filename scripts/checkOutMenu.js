...
function markLost(bookBarcode, bookName) {
    ...
    if (
        confirm(
            'Do you really want to report book ' +
                bookName +
                ' with barcode ' +
                bookBarcode +
                ' as lost?'
        ...
        xmlhttps.open(
            'POST',
            'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/library/lostBook',
            true
        );
...
function renewBook(bookBarcode) {
        ...
        if (this.readyState == 4 && this.status == 200) {
            if (this.responseText.trim() == 'Book renewed!') {
                ...
                renewBookList.push(
                    'book with barcode:' +
                        bookBarcode +
                        ' failed to renew because ' +
                        this.responseText +
                        '<br>'
                );
            }
            if (
                successRenewCount + failRenewCount == renewCount &&
                renewCount != 0
            ) {
                //display response
                var renewText = '';
                if (successRenewCount > 0) {
                    renewText +=
                        'successfully renewed ' + successRenewCount + ' book';
                    if (successRenewCount > 1) {
                        //if more that one book make it plural
                        renewText += 's';
                    }
                    renewText += '!<br>';
                }
    ...
    xmlhttps.open(
        'POST',
        'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/library/renewBooks',
        true
    );
    xmlhttps.setRequestHeader(
        'Content-type',
...
function userCheckedOut() {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            try {
                if (this.responseText.trim() == 'No rows') {
                    document.getElementById('checkout_table').innerHTML =
                        'You have no books checked out';
                    document.getElementById('renewButton').style.display =
                        'none'; //if no books hide button
                } else {
    ...
    var url =
        'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/user/userCheckedOut';
    xmlhttps.open('GET', url, true); //Set get request with given parameter
    xmlhttps.withCredentials = true;
    xmlhttps.send();
}
function createCheckOut(transactionJSON) {
    ...
    for (let transaction of transactionJSON) {
        ...
        tableText +=
            `<td><input type="checkbox" name='boxes[]' value ='` +
            bookBarcode +
            `'></td>`;
        //tableText += `<td><input type="checkbox" name='lostBoxes[]' value ='`+bookBarcode+`'></td>`;
        tableText +=
            '<td>' +
            `<input type='image' onclick='markLost("` +
            bookBarcode +
            `","` +
