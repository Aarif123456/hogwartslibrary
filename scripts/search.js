...
function searchBooks() {
    ...
    if (keyword != '') {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                try {
                    if (this.responseText.trim() != 'No rows') {
                        //no rows means no books were found
        ...
        var url =
            'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/library/searchCatalogue?';
...
function createBookTable(bookJSON) {
    //create table from json file
    // console.log(bookJSON);
    var key = ''; //force make key an string
    //create Table with given header
    var tableText =
        '<table><thead><tr><td></td><td>BookISBN</td><td>Title</td><td>Author</td><td>Pages</td><td>Edition</td><td>Status</td><td>Category</td><td>Holds</td></tr></thead>';
    tableText += '<tbody>'; //Start table body
    for (let book of bookJSON) {
        tableText += '<tr>'; //star new row
        if (
            book.hasOwnProperty('bookName') &&
            book.hasOwnProperty('bookISBN')
        ) {
            tableText +=
                '<td>' +
                `<button type="button" name='holdButton' onclick='holdBook("` +
                book['bookISBN'] +
                `","` +
                book['bookName'] +
                `");'>Hold Book</button></td>`;
        ...
        for (key in book) {
            //for each element in book create entry
            tableText += '<td>';
