...
function displayTotalFine(finesJSON) {
    ...
    if (totalFine > 0) {
        displayText +=
            ...
            totalFine +
            ' on your account.<br> You can pay your fine through a librarian during our available hours<br>';
    }
    if (blacklisted) {
        displayText +=
            'You are considered a blacklisted user which means that some of your privileges have been revoked until you pay your fines.<br>';
        displayText +=
            'A student is blacklisted if they carry a balance of $10 or more.<br>';
        displayText +=
            'A teacher is blacklisted if they carry a balance of $50 or more.<br>';
    }
...
function renderFineTable(update) {
    ...
    var tableText = '';
    if (!update) {
        //if we have new table reset page to 0
        setPage(0, 0);
    }
    if (numRows == 0) {
    ...
    var endIndex = Math.min(
        (finePageNum + 1) * numRows,
        fineTable[mode].length
    );
    var s = fineTable[mode].slice(finePageNum * numRows, endIndex);
    if (s.join().includes('<tr>')) {
        //if the dictionary contains a table then create a table
        ...
        //other display the user Text fully
...
function createRelevantFines(finesJSON) {
    ...
    if (totalFine === -1) {
        //waiting for ajax to call to change total fine value
        setTimeout(function () {
            createRelevantFines(finesJSON);
    ...
    <td>Return/lost date</td><td>Fine</td>`;
        ...
        for (let fine of finesJSON) {
            try {
                //fines are in descending order, so newest comes first
            ...
            if (fine['lostDate'] == null) {
                //if it wasn't lost just give return date
                ...
                //make lost books shine
                tableText +=
                    "<td class='lostChild'>" + fine['lostDate'] + '</td>';
            }
...
function createLostBooks(finesJSON) {
  ...
  <td>lost date</td><td>Price></td><td>Fine</td>`;
    ...
    for (let fine of finesJSON) {
        if (fine['lostDate'] == null) {
            //if not lost then it is not relevant
            continue;
        }
        tableText += '<tr>';
        tableText += '<td>' + fine['bookName'] + '</td>';
...
function userFines(listType) {
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            try {
                if (this.responseText.trim() == 'No rows') {
                    document.getElementById('fine_table').innerHTML =
                        'You have no fines at the moment!';
                    document.getElementById('frontPageButton').style.display =
                        'none';
                    document.getElementById('backPageButton').style.display =
                        'none';
                } else {
                    if (listType.trim() == 'getTransactionWithFines') {
                        createFineTable(JSON.parse(this.responseText));
                    } else if (
                        listType.trim() == 'getOutstandingFineOnAccount'
                    ) {
    ...
    var url =
        'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/user/userFines?listType=' +
        listType;
