...
function deleteLibrarian() {
    ...
    var url =
        'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/headmaster/deleteLibrarian';
...
function addLibrarian() {
    ...
    var xmlhttp = new XMLHttpRequest();
    var url =
        'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/headmaster/addLibrarian';
    xmlhttp.open('POST', url, true);
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            console.log(this.responseText);
...
function loadLibrarianList(listName) {
    //use Ajax to get list of librarian or potential librarian
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            try {
...
function getUserList(userJSON) {
    //find potential librarian from json file
    var userListText =
        "<select id='librarianSelection' form = 'updateLibrarian'>";
    for (let user of userJSON) {
        ...
        userListText +=
            user['memberID'] + ':     ' + user['fname'] + ' ' + user['lname'];
        userListText += '</option> ';
    }
