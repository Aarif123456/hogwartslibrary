...
function registerUser(valid) {
    ...
    if (valid === false) {
        ...
        var url =
            'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/userManagement/addUser';
...
function checkUsername(callback) {
    ...
    if (!/^[a-zA-Z0-9_]+$/.test(username)) {
        document.getElementById('hint').innerHTML =
            'Username can only be alphabets, numbers or underscores';
        return false;
    }
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
    ...
    var url =
        'https://arif115.myweb.cs.uwindsor.ca/hogwartslibrary/api/userManagement/checkUserName';
...
function displayFields() {
    ...
    if (userType[0].checked) {
        //run if student selected

        userCategoryText.innerHTML =
            "<label for='userCategoryText' >Major </label><br>";
    ...
    if (userType[1].checked) {
        //code if professor is selected
        userCategoryText.innerHTML =
            "<label for='userCategoryText' >Department </label><br>";
